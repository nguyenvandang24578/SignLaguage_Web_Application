...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=3, drop_out=0):
        ...
        self.layer11        = STA_GCN(4*base_channels, 256, A_norm, A)
    ...
    def forward(self, x, keep_prob=None, is_feat=False):
        if len(x.shape) == 3:
            N, T, VC = x.shape
...
#     import sys
#     sys.path.append('..')
#     model = Model(num_class=226, num_point=25, num_person=2, graph=graph)
#     N, C, T, V, M = 10, 3, 150, 25, 2
#     x = torch.randn(N,C,T,V,M)
#     model.forward(x)
#     print('Model total # params:', count_params(model))
def sta_gcn_joint(num_classes):
    return Model(
        num_class=num_classes,
        num_point=27,
        num_person=1,
        graph='graph_stagcn.sign_27.Graph'
    )
def sta_gcn_joint_wlasl100(num_classes):
    return Model(
        num_class=num_classes,
        num_point=27,
        num_person=1,
...
def sta_gcn_joint_motion(num_classes):
    return Model(
        num_class=num_classes,
        num_point=27,
        num_person=1,
...
def sta_gcn_bone_motion(num_classes):
    return Model(
        num_class=num_classes,
        num_point=27,
        num_person=1,
